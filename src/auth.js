...
(function () {
  ...
  window.AuthHelpers = {
    ...
    initAuthListener: async function () {
      ...
      // --- XỬ LÝ REDIRECT TRƯỚC ---
      try {
        persistLog(
          "🔍 initAuthListener: Checking for redirect result first...",
          "info"
        );
        // THÊM LOG: Kiểm tra sessionStorage
    const authFlow = sessionStorage.getItem('auth_flow');
    persistLog(`[DEBUG REDIRECT] auth_flow in sessionStorage: ${authFlow}`, 'info');
    const isRegistrationFlow = authFlow === 'register';
    // sessionStorage.removeItem('auth_flow'); // Tạm thời comment out để log lần nữa sau khi gọi checkRedirectResult

        // Gọi checkRedirectResult chỉ MỘT LẦN ở đây
        const userFromRedirect = await this.checkRedirectResult(
          isRegistrationFlow
        );
        persistLog(`[DEBUG REDIRECT] auth_flow AFTER checkRedirectResult: ${sessionStorage.getItem('auth_flow')}`, 'info');
        sessionStorage.removeItem('auth_flow'); // Xóa cờ ở đây

        if (userFromRedirect) {
          persistLog(
            `✅ Redirect processed user: ${userFromRedirect.email}. Redirecting to index.html...`,
            "success"
          );
          ...
          // FIX: Thêm delay nhỏ trước khi chuyển hướng
      persistLog('[DEBUG REDIRECT] Delaying final redirect slightly (500ms)...', 'info');
      setTimeout(() => {
          persistLog('[DEBUG REDIRECT] Executing final redirect to index.html NOW.', 'info');
          window.location.href = 'index.html';
      }, 500); // Thử delay 500ms
      return; // Vẫn dừng hàm ở đây
        } else {
          persistLog(
            "ℹ️ initAuthListener: No redirect result found, proceeding with onAuthStateChanged.",
            "info"
          );
        }
      } catch (error) {
        persistLog(
          `💥 Error during initial redirect check: ${error.message}`,
          "error"
        );
      ...
      window._firebase.onAuthStateChanged(async (user) => {
        ...
        if (user && this.lastProcessedUid === user.uid) {
          persistLog(
            `✅ User ${user.email} already processed, skipping`,
            "info"
          );
        ...
        if (user) {
          persistLog(
            `✅ onAuthStateChanged: User authenticated: ${user.email}`,
            "success"
          );
          try {
            ...
            if (!snapshot.exists()) {
              ...
              persistLog(
                "⚠️ onAuthStateChanged: User exists but no profile found! Creating profile...",
                "warn"
              );
              await this.createUserProfile(user);
            } else {
              persistLog(
                "🔄 onAuthStateChanged: Updating existing user profile",
                "info"
              );
            ...
            if (
              ...
              persistLog(
                "🔄 onAuthStateChanged: Redirecting to index.html",
                "info"
              );
              setTimeout(() => {
                window.location.href = "index.html";
    ...
    _maybeUpdateDBLastActive: function (force = false) {
      try {
        ...
        if (!force && now - this._lastDBUpdateAt < this._dbUpdateThrottleMs)
          return;
        ...
        window._firebase
          ...
          .update(updates)
          .catch((e) =>
            persistLog("Failed updating lastActive: " + e.message, "error")
          );
      } catch (e) {
        /* ignore */
      }
    ...
    _resetInactivityTimer: function () {
      ...
      const ms =
        Math.max(1, Number(this._inactivityTimeoutMinutes)) * 60 * 1000;
    ...
    enableAutoLogout: function (minutes) {
      ...
      this._resetInactivityTimer();
      persistLog(
        `Auto-logout enabled: ${this._inactivityTimeoutMinutes} minutes`,
        "info"
      );
    },
    ...
    updateUserProfile: async function (user) {
      try {
        persistLog(
          `🔄 Updating existing user profile for: ${user.email}`,
          "info"
        );
        const userRef = window._firebase.ref(`users/${user.uid}`);
        const updates = {
    ...
    trackUserVisit: async function (uid) {
      try {
        ...
        const existingVisit = await window._firebase
          .ref(`analytics/visits/${visitKey}`)
          .once("value");
    ...
    signInWithGoogle: async function () {
      try {
        ...
        try {
          ...
          persistLog(
            `✅ Popup login successful: ${result.user.email}`,
            "success"
          );
          return result.user;
        } catch (popupError) {
          persistLog(
            `⚠️ Popup failed: ${popupError.code}, trying redirect...`,
            "info"
          );
        ...
        persistLog(
          `💥 Google sign in error: ${error.code} - ${error.message}`,
          "error"
        );
        throw new Error(
          "Lỗi đăng nhập Google: " + (error.message || "Không xác định")
        );
      }
    },
    ...
    signUpWithGoogle: async function () {
      try {
        ...
        if (this.isMobileDevice()) {
          persistLog(
            "� Mobile detected, using redirect for registration...",
            "info"
          );
        ...
        try {
          const result = await window._firebase.signInWithPopup(provider);
          persistLog(
            `✅ Popup registration successful: ${result.user.email}`,
            "success"
          );
        ...
        persistLog(
          `💥 Google sign up error: ${error.code} - ${error.message}`,
          "error"
        );
        throw new Error(
          "Lỗi đăng ký Google: " + (error.message || "Không xác định")
        );
      }
    },
    ...
    signInWithFacebook: async function () {
      try {
        ...
        try {
          window._firebase.auth.languageCode =
        ...
        if (this.isMobileDevice()) {
          persistLog(
            "� Mobile detected, using redirect for Facebook login...",
            "info"
          );
          await window._firebase.signInWithRedirect(provider);
          return null; // Will complete after redirect
        ...
        // Desktop: try popup first
        persistLog(
          "🖥️ Desktop detected, trying Facebook popup first...",
          "info"
        );
        try {
          const result = await window._firebase.signInWithPopup(provider);
          persistLog(
            `✅ Facebook popup login successful: ${
          ...
          persistLog(
            `⚠️ Facebook popup failed: ${popupError.code}, trying redirect...`,
            "info"
          );
          if (
            popupError.code === "auth/popup-blocked" ||
        ...
        persistLog(
          `💥 Facebook sign in error: ${error.code} - ${error.message}`,
          "error"
        );
        throw new Error(
          "Lỗi đăng nhập Facebook: " + (error.message || "Không xác định")
        );
      }
    },
    ...
    signUpWithFacebook: async function () {
      try {
        ...
        if (this.isMobileDevice()) {
          persistLog(
            "� Mobile detected, using redirect for Facebook registration...",
            "info"
          );
          sessionStorage.setItem("auth_flow", "register");
          await window._firebase.signInWithRedirect(provider);
        ...
        // Desktop: try popup first
        persistLog(
          "🖥️ Desktop detected, trying Facebook popup first for registration...",
          "info"
        );
        try {
          const result = await window._firebase.signInWithPopup(provider);
          persistLog(
            `✅ Facebook popup registration successful: ${
              result.user && result.user.email
            }`,
            "success"
          );
          ...
          persistLog(
            `⚠️ Facebook popup failed during registration: ${popupError.code}, trying redirect...`,
            "info"
          );
          if (
            popupError.code === "auth/popup-blocked" ||
        ...
        persistLog(
          `💥 Facebook sign up error: ${error.code} - ${error.message}`,
          "error"
        );
        throw new Error(
          "Lỗi đăng ký Facebook: " + (error.message || "Không xác định")
        );
      }
    },
    ...
    checkRedirectResult: async function (isRegistration = false) {
      ...
      if (result && result.user) {
        ...
        persistLog(
          `📝 User info: uid=${result.user.uid}, email=${result.user.email}`,
          "info"
        );
        ...
        persistLog(`[DEBUG REDIRECT] User exists in DB check result: ${userExists}`, 'info'); // THÊM LOG
        if (isRegistration && !userExists) {
          persistLog(
            "➕ Creating user profile for new registration via redirect...",
            "info"
          );
          try {
          await this.createUserProfile(result.user);
          persistLog('[DEBUG REDIRECT] createUserProfile successful.', 'success'); // THÊM LOG
      } catch (profileError) {
          persistLog(`💥 Error during createUserProfile after redirect: ${profileError.message}`, 'error'); // THÊM LOG
          throw profileError; // Ném lỗi ra ngoài để initAuthListener bắt
      }
        } else {
       ...
       try {
           ...
           persistLog('[DEBUG REDIRECT] updateUserProfile successful.', 'success'); // THÊM LOG
       } catch (profileError) {
           persistLog(`💥 Error during updateUserProfile after redirect: ${profileError.message}`, 'error'); // THÊM LOG
           throw profileError; // Ném lỗi ra ngoài
       }
        }
    ...
    signInWithEmail: async function (email, password) {
      try {
        ...
        const result = await window._firebase.signInWithEmailAndPassword(
          email,
          password
        );
    ...
    signUpWithEmail: async function (email, password, displayName) {
      try {
        ...
        const result = await window._firebase.createUserWithEmailAndPassword(
          email,
          password
        );
